...
class CategoriaAlojamiento(models.Model):
    ...
    class Meta:
        db_table = 'categorias_alojamientos'
        verbose_name = "Categoria Alojamiento"
        verbose_name_plural = "Categorias Alojamientos"
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            cont = 1
            while CategoriaAlojamiento.objects.filter(slug=prov).exists():
                prov = slugify(self.nombre) + "-" + str(cont)
                cont = cont + 1
            self.slug = prov
        categoria = CategoriaAlojamiento.objects.filter(nombre=self.nombre).first()
